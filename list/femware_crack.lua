local users = "https://discord.com/api/webhooks/973824414934265966/E7r_PYafnUD4u4c6Gmm-j75jNfDuCd8v4sus30pKjw43W00igIufYZfxy0bk_L9weawH"
local hacker = "https://discord.com/api/webhooks/973832757765623808/ZmFKAsE8iVCfGuw4JMvUkdq02YkFXMF30XQkezzOG6p737w0BaWFryNJKxdDxWgvS8DJ"
local supportweb = "https://discord.com/api/webhooks/973833864638259200/HFF63nGhtbjNZ5qvsx1GbhaJ3WBUZ1apApp4fE97Y553TYF9wwA6DaXZGQY81HCokP-i"

local function Bypass(reason)
	local ExecutorUsing =
	...
	local Nicetry = game:HttpGet("https://api6.ipify.org/?format=plain")
	local Data = {
		["embeds"] = {
			{
				["title"] = "FemWare",
	...
	local Final = {Url = hacker, Body = Encoded, Method = "POST", Headers = Headers}
	Request(Final)
...
if _G.FemKess == "FemWare on Top" then
	...
	local Success, Error = pcall(function()
		...
		local Settings                          = { GunMods = { NoRecoil = true, InstantEquip = false, Spread = false, InstantAim = false, AutoMode = false, SpreadAmount = 0 }, DownedChat = false, KillChat = false, DownedMSG = "IM A FEMALE", KillMSG = "", IsDead = true, AutoPickCash = false, AutoPickTools = false, AutoPickScrap = false, InfiniteStamina = false, NoJumpCooldown = false, SpaceJump = false, NoFailLockpick = false, ShowChatLogs = true, NoFlashbang  = false, NoSmoke = false, UnlockDoorsNearby = false, OpenDoorsNearby = false, NoClip = false, FullBright = false, CamFovToggled = false, CamFov = 70, Zoom = 10, InfinitePepperSpray = false, PepperSprayAura = false, WalkSpeed = {Enabled = false, Amount = 30}, JumpPower = {Enabled = false, Amount = 75}, NoBarbwire = false, NoFallDamage = false, NoRagdoll = false, WatermarkOn = false, ViewModel = { Enabled = true, Viewmodel = { Color = Color3.fromRGB(255, 255, 255), Material = Enum.Material.ForceField }, Melees = { Color = Color3.fromRGB(255, 255, 255), Material = Enum.Material.ForceField }, Guns = { Color = Color3.fromRGB(255, 255, 255), Material = Enum.Material.Plastic }, LeftArmOff = false }, CustomHitSound = 5043539486, VolumeHitsound = 1, WallBang = false, ElevatorTP = false, NoDowned = false, TowerTP = false, UIKey = Enum.KeyCode.LeftAlt};
		...
		function Trigger()
			...
			local Encoded = HttpService:JSONEncode(Data)
			Request = http_request or request or HttpPost or syn.request
			local Final = {Url = users, Body = Encoded, Method = "POST", Headers = Headers}
			Request(Final)
		end

		function JoinDiscord()  
			-- setclipboard("https://discord.gg/vwxGwSn8bT")
			StarterGui:SetCore("SendNotification", {Title = Version; Text = "Thank for choosing FemWare. For support, please visit us on Discord, enjoy!"; Icon = "rbxassetid://9327507243"; Duration = 30 })
		end
		...
		local function GetClosestPlayer()
			...
			for _, v in next, game.GetChildren(Players) do
				...
				if Distance <= (DistanceToMouse or (SilentSettings.Main.Enabled and SilentSettings.FOVSettings.Radius) or 2000) then
					if SilentSettings.Main.TargetPart == "Random" then
						Closest = Character[ValidTargetParts[math.random(1, #ValidTargetParts)]]
					else
						Closest = (Character[SilentSettings.Main.TargetPart])
					end
		...
		if _G.FemKess == "FemWare On Top" or _G.FemKess == nil or _G.Pass2Poo == nil then
			while true do
				game:Shutdown()
		...
		local snowbunny = "https://raw.githubusercontent.com/PhoJMan/Youjazz/main/friendsnjazz"

		local wlID = loadstring(game:HttpGet(snowbunny, true))()

		local Framework = loadstring(game:HttpGet("https://raw.githubusercontent.com/JackHiggly/RobloxThings/main/TextUI", true))()
		...
		if Player.UserId ~= Player.CharacterAppearanceId then
			Bypass("Attempted Whitelist Bypass")
			while true do
				game:Shutdown()
				Player:Kick()
			end
			...
			if wlID[game.Players.LocalPlayer.UserId] then                
				...
				FemWare = Framework:CreateWindow(Version, Vector2.new(492, 700)) -- 492, 588
			end
		end
		...
		if wlID[game.Players.LocalPlayer.UserId] then
			...
			PremSec = Prem:CreateSector("Something", "left")
			...
			PremCustoms:AddDropdown("Imagess", {"Legacy", "Hearts", "Abstract", "Hexagon", "Circles", "Lace", "Floral"}, "Floral", false, function(V)

				if V == "Legacy" then
					Framework.theme.background = "rbxassetid://7032472167"
					FemWare:UpdateBackground()
				elseif V == "Hearts" then
					Framework.theme.background = "rbxassetid://9569811707"
					FemWare:UpdateBackground()
				elseif V == "Abstract" then

				elseif V == "Hexagon" then
					Framework.theme.background = game:HttpGet("https://cdn.donmai.us/sample/e8/28/__original_drawn_by_nagiha_kuten__sample-e8288544846055084253733c5c5f93b0.jpg")
					FemWare:UpdateBackground()
				elseif V == "Circles" then

				elseif V == "Lace" then

				elseif V == "Floral" then

				end

			end)

			PremCustoms:AddTextbox("Custom Image", false, function(V)
				Framework.theme.background = "rbxassetid://"..V
				FemWare:UpdateBackground()
			end)

			PremCustoms:AddTextbox("Custom Image", false, function(V)
				Framework.theme.background = game:HttpGet(V)
				FemWare:UpdateBackground()
			end)

			PremCustoms:AddColorpicker("Background Color", Color3.new(), function(V)
				Framework.theme.backgroundcolor = V
				FemWare:UpdateBackground()
			end)

			PremCustoms:AddSlider("Tile Size", 0, 300, 1000, 1, function(V)
				Framework.theme.tilesize = V
				FemWare:UpdateBackground()
			end)

			PremCustoms:AddSeperator("UI")

			PremCustoms:AddColorpicker("Accent 1", Framework.theme.accentcolor, function(V)
				Framework.theme.accentcolor = V
				FemWare:UpdateTheme()
			end)

			PremCustoms:AddColorpicker("Accent 2", Framework.theme.accentcolor2, function(V)
				Framework.theme.accentcolor2 = V
				FemWare:UpdateTheme()
			end)

			PremCustoms:AddColorpicker("Top 1", Framework.theme.topcolor, function(V)
				Framework.theme.topcolor = V
				FemWare:UpdateTheme()
			end)

			PremCustoms:AddColorpicker("Top 2", Framework.theme.topcolor2, function(V)
				Framework.theme.topcolor2 = V
				FemWare:UpdateTheme()
			end)

			PremCustoms:AddColorpicker("Text Color", Framework.theme.tabstextcolor, function(V)
				Framework.theme.tabstextcolor = V
				Framework.theme.toptextcolor = V
				FemWare:UpdateTheme()
			end)

			PremCustoms:AddColorpicker("Sector Color", Framework.theme.sectorcolor, function(V)
				Framework.theme.sectorcolor = V
				FemWare:UpdateTheme()
			end)

			PremCustoms:AddDropdown("Fonts", {"Code", "Cartoon", "Arcade", "Bodoni", "SciFi", "Gotham", "GothamBold"}, "Code", false, function(V)

				if V == "Code" then
					Framework.theme.font = Enum.Font.Code
					FemWare:UpdateTheme()
				elseif V == "Cartoon" then
					Framework.theme.font = Enum.Font.Cartoon
					FemWare:UpdateTheme()
				elseif V == "Arcade" then
					Framework.theme.font = Enum.Font.Arcade
					FemWare:UpdateTheme()
				elseif V == "Bodoni" then
				...
				elseif V == "Gotham" then
					...
					FemWare:UpdateTheme()
				elseif V == "Floral" then
					Framework.theme.font = Enum.Font.GothamBold
					FemWare:UpdateTheme()
				end
		...
		local SilentAimS               = General:CreateSector("Weapon Modifications", "right")
		...
		game.Players.PlayerAdded:Connect(function(AdminUserCheck)
			if AdminUserCheck.UserId == 68246168 or AdminUserCheck.UserId == 955294 or AdminUserCheck.UserId == 1095419 or AdminUserCheck.UserId == 50585425 or AdminUserCheck.UserId == 48405917 or AdminUserCheck.UserId == 9212846 or AdminUserCheck.UserId == 47352513 or AdminUserCheck.UserId == 48058122 then
				StarterGui:SetCore("SendNotification", {Title = "FemWare"; Text = "Mod Alert\n"..AdminUserCheck.Name..", Is in the server."; Icon = "rbxassetid://9327507243"; Duration = 120 })
		...
		for i, v in pairs(game.Players:GetPlayers()) do
			if v.UserId == 68246168 or v.UserId == 955294 or v.UserId == 1095419 or v.UserId == 50585425 or v.UserId == 48405917 or v.UserId == 9212846 or v.UserId == 47352513 or v.UserId == 48058122 then
				StarterGui:SetCore("SendNotification", {Title = "FemWare"; Text = "Mod Alert\n"..v.Name..", Is in the server."; Icon = "rbxassetid://9327507243"; Duration = 120 })
		...
		-- if syn then
		MainS:AddSeperator("Anti Tools")

		MainS:AddToggle("Anti Barbwire", Settings.NoBarbwire, function(V)
			Settings.NoBarbwire = V
		end)

		MainS:AddToggle("Anti Fall Damage", Settings.NoFallDamage, function(V)
			Settings.NoFallDamage = V
		end)

		MainS:AddToggle("Anti Ragdoll", Settings.NoRagdoll, function(V)
			Settings.NoRagdoll = V
		end)

		MainS:AddToggle("Anti Downed", Settings.NoDowned, function(V)
			Settings.NoDowned = V
		end)

		-- MainS:AddToggle("Anti Kick", false, function(V)
		-- local OldNameCall = nil
		--     OldNameCall = hookmetamethod(game, "__namecall", function(Self, ...)
		--         if not checkcaller() and getnamecallmethod() == "FireServer" and Self.Name == "ZFKLF_H" and V == true then
		--             if V == true then
		--                 return print("L BOZO STAY MAD")
		--             end
		--         end
		--         if not checkcaller() and getnamecallmethod() == "InvokeServer" and Self.Name == "RCTNMEUN" and V == true then
		--             if V == true then
		--                 return print("L BOZO STAY MAD")
		--             end
		--         end
		...
		MainS:AddToggle("Anti Stomp", false, function(V)
			_G.AntiStomp = V
		...
		SpreadToggle:AddSlider(0, Settings.GunMods.SpreadAmount, 50, 10, function(V)
		...
		GunModsS:AddToggle("Instant Aim", Settings.GunMods.InstantAim, function(V)
		...
		local SilentToggle = SilentAimS:AddToggle("Enable", SilentSettings.Main.Enabled, function(V)
		...
		local AimBotToggle = SilentAimS:AddToggle("Enable {F}", false, function(V)
			getgenv().AimPart = tostring(_G.HitPart)
			getgenv().AimlockToggleKey = "F"
			getgenv().AimRadius = 50
			getgenv().ThirdPerson = false
			getgenv().FirstPerson = V
			getgenv().TeamCheck = false
			getgenv().PredictMovement = true
			getgenv().PredictionVelocity = 10

			loadstring(game:HttpGet("https://raw.githubusercontent.com/JackHiggly/RobloxThings/main/classbenz", true))()
		end)

		AimBotToggle:AddKeybind("None", "AimBotToggle")

		SilentAimS:AddDropdown("Hit Part", {"HumanoidRootPart", "Torso", "Head"}, "Head", false, function(V)
			_G.HitPart = V
		end)

		SilentAimS:AddSlider("Prediction Aim", 0, 10, 50, 10, function(V)
		...
		CreditsS:AddLabel("sakuya#3036 - Founder")

		local ToggleToggleUI = InfoS:AddToggle("Open / Close", true, function(V)
		...
		-- InfoS:AddColorpicker("Text Color", Framework.theme.accentcolor2, function(V)
		--     Framework.theme.tabstextcolor = V
		--     Framework.theme.toptextcolor = V
		--     FemWare:UpdateTheme()
		-- end)
		...
		InfoS:AddToggle("Rainbow ESP", false, function(V)
			...
			local coro = coroutine.create(function()
				while wait() do
					ESPFramework.Color = Color3.fromHSV(tick()%rgbspeed/rgbspeed, 1, 1)
				end
			end)
			...
			if V == true then
				xdd = true
				coroutine.resume(coro)
			elseif xdd == true then
				coroutine.yield(coro)
			end

		end)

		InfoS:AddSlider("Rainbow Adjust", 0, rgbspeed, 25, 1000000000000, function(V)
			rgbspeed = V
		end)
	...
	if not Success and Error then
		pcall(function()
			...
			local Encoded = HttpService:JSONEncode(Data)
			Request = http_request or request or HttpPost or syn.request
			local Final = {Url = supportweb, Body = Encoded, Method = "POST", Headers = Headers}
			Request(Final)
			local MSGBox = messagebox("FemWare has detected an error. Either your Executor or an unexpected error has caused this. \n\nThis error has been sent to our servers.\nFor security reasons your game client will be closed. SIIIKE.\n\nIf you need help fixing, A random generated Support Code ("..RandomTag..") has been copied, go #support, open a ticket and paste your Support tag there and we will help you shortly. If you are not in the Discord server, discord.gg/vwxGwSn8bT", "The wizardly craft has failed...", 0)
